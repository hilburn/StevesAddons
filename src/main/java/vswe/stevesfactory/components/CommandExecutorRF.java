...
public class CommandExecutorRF extends CommandExecutor {
        ...
        if(!this.usedCommands.contains(Integer.valueOf(command.getId()))) {
            try {
                ...
                switch(command.getType().ordinal()) {
                    case 1:
                        List inputInventory = this.getInventories((ComponentMenu)command.getMenus().get(0));
                        if(inputInventory != null) {
                            this.getValidSlots((ComponentMenu)command.getMenus().get(1), inputInventory);
                            this.getItems((ComponentMenu)command.getMenus().get(2), inputInventory);
                        }
                        break;
                    case 2:
                        List outputInventory = this.getInventories((ComponentMenu)command.getMenus().get(0));
                        if(outputInventory != null) {
                            this.getValidSlots((ComponentMenu)command.getMenus().get(1), outputInventory);
                            this.insertItems((ComponentMenu)command.getMenus().get(2), outputInventory);
                        }
                        break;
                    case 3:
                        List conditionInventory = this.getInventories((ComponentMenu)command.getMenus().get(0));
                        if(conditionInventory != null) {
                            this.getValidSlots((ComponentMenu)command.getMenus().get(1), conditionInventory);
                        ...
                        return;
                    case 4:
                        List inputTank = this.getTanks((ComponentMenu) command.getMenus().get(0));
                        if(inputTank != null) {
                        ...
                        break;
                    case 5:
                        List outputTank = this.getTanks((ComponentMenu) command.getMenus().get(0));
                        if(outputTank != null) {
                        ...
                        break;
                    case 6:
                        List conditionTank = this.getTanks((ComponentMenu) command.getMenus().get(0));
                        ...
                        return;
                    case 7:
                        if(ComponentMenuSplit.isSplitConnection(command) && this.splitFlow((ComponentMenu)command.getMenus().get(0))) {
                            return;
                        }
                        break;
                    case 8:
                        List emitters = this.getEmitters((ComponentMenu)command.getMenus().get(0));
                        if(emitters != null) {
                        ...
                        break;
                    case 9:
                        List nodes = this.getNodes((ComponentMenu)command.getMenus().get(0));
                        if(nodes != null) {
                        ...
                        return;
                    case 10:
                        List tiles = this.getTiles((ComponentMenu)command.getMenus().get(2));
                        if(tiles != null) {
                    ...
                    case 14:
                        ...
                        return;
                    case 15:
                        List camouflage = this.getCamouflage((ComponentMenu)command.getMenus().get(0));
                        if(camouflage != null) {
                            ...
                            if(items.isFirstRadioButtonSelected() || ((Setting)items.getSettings().get(0)).isValid()) {
                        ...
                        break;
                    case 16:
                        List sign = this.getSign((ComponentMenu)command.getMenus().get(0));
                        if(sign != null) {
                        ...
                        break;
                    case 17:
                        List inputStorage = this.getRFStorage((ComponentMenu) command.getMenus().get(0));
                        if(inputStorage != null) {
                            this.getInputRF((ComponentMenu) command.getMenus().get(1), inputStorage);
                        }
                        break;
                    case 18:
                        List outputStorage= this.getRFStorage((ComponentMenu) command.getMenus().get(0));
                        if(outputStorage != null) {
                            this.insertRF((ComponentMenu) command.getMenus().get(1), outputStorage);
                        }
                        break;
                    case 19:
                        List conditionStorage = this.getRFStorage((ComponentMenu) command.getMenus().get(0));
    ...
    private List<SlotInventoryHolder> getTiles(ComponentMenu componentMenu) {
        return getContainers(this.manager, componentMenu, (ConnectionBlockType)null);
    }
    ...
    public static List<SlotInventoryHolder> getContainers(TileEntityManager manager, ComponentMenu componentMenu, ConnectionBlockType type) {
        ...
        if(menuContainer.getSelectedInventories().size() == 0) {
            ...
            for(i = 0; i < menuContainer.getSelectedInventories().size(); ++i) {
                int var14 = ((Integer)menuContainer.getSelectedInventories().get(i)).intValue() - VariableColor.values().length;
    ...
    private static void addContainer(List<ConnectionBlock> inventories, List<SlotInventoryHolder> ret, int selected, ComponentMenuContainer menuContainer, ConnectionBlockType requestType, EnumSet<ConnectionBlockType> variableType) {
        if(selected >= 0 && selected < inventories.size()) {
            ConnectionBlock connection = (ConnectionBlock)inventories.get(selected);
    ...
    private void getValidSlots(ComponentMenu componentMenu, List<SlotInventoryHolder> inventories) {
        ...
        for(int i = 0; i < inventories.size(); ++i) {
            IInventory inventory = ((SlotInventoryHolder)inventories.get(i)).getInventory();
            Map validSlots = ((SlotInventoryHolder)inventories.get(i)).getValidSlots();
    ...
    private void getValidRFStorage(ComponentMenu componentMenu, List<SlotInventoryHolder> cells) {
        ...
        for(int i = 0; i < cells.size(); ++i) {
            IEnergyHandler cell = (IEnergyHandler)((SlotInventoryHolder) cells.get(i)).getTile();
    ...
    private void getInputRF(ComponentMenu componentMenu, List<SlotInventoryHolder> inputStorage)
    {
        ...
        for(int i = 0; i < inputStorage.size(); ++i) {
            ...
            if (cell instanceof TileEntityRFNode) ((TileEntityRFNode)cell).setInputSides(validSides.toArray(new Integer[validSides.size()]));
    ...
    private void insertRF(ComponentMenu componentMenu, List<SlotInventoryHolder> outputStorage)
    {
        ...
        for (SlotInventoryHolder holder:outputStorage)
        {
            ...
            if (cell==null)continue;
            if (cell instanceof TileEntityRFNode) ((TileEntityRFNode)cell).setOutputSides(validSides.toArray(new Integer[validSides.size()]));
            for(int side :validSides){
    ...
    private void getValidTanks(ComponentMenu componentMenu, List<SlotInventoryHolder> tanks) {
        ...
        for(int i = 0; i < tanks.size(); ++i) {
            IFluidHandler tank = ((SlotInventoryHolder)tanks.get(i)).getTank();
            Map validTanks = ((SlotInventoryHolder)tanks.get(i)).getValidSlots();
    ...
    private boolean searchForStuff(ComponentMenu componentMenu, List<SlotInventoryHolder> inventories, boolean useLiquids) {
        int i;
        if(!((SlotInventoryHolder)inventories.get(0)).isShared()) {
            boolean var7 = ((SlotInventoryHolder)inventories.get(0)).getSharedOption() == 1;

            for(i = 0; i < inventories.size(); ++i) {
                HashMap conditionSettingCheckerMap = new HashMap();
                this.calculateConditionData(componentMenu, (SlotInventoryHolder)inventories.get(i), conditionSettingCheckerMap, useLiquids);
            ...
            for(i = 0; i < inventories.size(); ++i) {
                this.calculateConditionData(componentMenu, (SlotInventoryHolder)inventories.get(i), useAnd, useLiquids);
            }
    ...
    private void calculateConditionDataItem(ComponentMenu componentMenu, SlotInventoryHolder inventoryHolder, Map<Integer, ConditionSettingChecker> conditionSettingCheckerMap) {
        ...
        while(i$.hasNext()) {
            ...
            if(this.isSlotValid(inventoryHolder.getInventory(), itemStack, slot, true)) {
                ...
                if(setting != null) {
                    ConditionSettingChecker conditionSettingChecker = (ConditionSettingChecker)conditionSettingCheckerMap.get(Integer.valueOf(setting.getId()));
                    ...
                    for(i$2 = 0; i$2 < len$; ++i$2) {
                        ...
                        if(fluidTankInfo != null) {
                            ...
                            if(!alreadyUsed) {
                                ...
                                if(var19 != null) {
                                    ConditionSettingChecker conditionSettingChecker = (ConditionSettingChecker)conditionSettingCheckerMap.get(Integer.valueOf(var19.getId()));
                                    if(conditionSettingChecker == null) {
