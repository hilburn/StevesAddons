...
public class TileEntityRFCluster extends TileEntityCluster implements IEnergyHandler
{
    ...
    public void writeData(DataWriter dw, EntityPlayer player, boolean onServer, int id)
    {
        ...
        } else if (onServer)
        {
            ...
            {
                dw.writeData((byte) this.registryList.get(i).getId(), DataBitHelper.CLUSTER_SUB_ID);
            }

            for (i = 0; i < this.elements.size(); ++i)
            {
                dw.writeData((byte) this.elements.get(i).func_145832_p(), DataBitHelper.BLOCK_META);
            }
        }
