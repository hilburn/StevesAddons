...
public class WideNumberBoxList {
    ...
    public void onClick(int mX, int mY, int button) {
        ...
        while (i$.hasNext()) {
            WideNumberBox textBox = (WideNumberBox) i$.next();
            if (textBox.isVisible()
                    && CollisionHelper.inBounds(textBox.getX(), textBox.getY(), textBox.getWidth(), 12, mX, mY)) {
    ...
    public boolean onKeyStroke(GuiManager gui, char c, int k) {
        if (this.selectedTextBox != null && this.selectedTextBox.isVisible()) {
            ...
            if (Character.isDigit(c)) {
                ...
                if ((double) Math.abs(this.selectedTextBox.getNumber())
                        < Math.pow(10.0D, (double) (this.selectedTextBox.getLength() - 1))) {
                    this.selectedTextBox.setNumber(
                            Math.min(this.selectedTextBox.getNumber() * 10 + i, this.selectedTextBox.getMaxNumber()));
                    this.selectedTextBox.onNumberChanged();
                }
